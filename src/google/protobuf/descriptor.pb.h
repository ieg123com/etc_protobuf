...
class LIBPROTOBUF_EXPORT DescriptorProto : public ::google::protobuf::Message /* @@protoc_insertion_point(class_definition:google.protobuf.DescriptorProto) */ {
 public:
  ...
  std::string* mutable_final_comment();
  const string& final_comment()const;

  ::std::string* mutable_head_comment();
  const ::std::string& head_comment()const;

  // returned self the response message name.
 ...
 private:
  ...
  mutable int _cached_size_;
  ::google::protobuf::internal::ArenaStringPtr name_;
  string head_comment_;
  string final_comment_;
...
inline string* DescriptorProto::mutable_final_comment() {
	return &final_comment_;
}

inline const string& DescriptorProto::final_comment() const {
    return final_comment_;
}

inline ::std::string* DescriptorProto::mutable_head_comment()
{
    return &head_comment_;
}

inline const ::std::string& DescriptorProto::head_comment()const
{
    return head_comment_;
}

inline const ::std::string DescriptorProto::response_type()const
{
    ::std::string result,buffer;
    auto all_comments = Split(head_comment_, "\n");
    if (all_comments.empty())return "";
    // 注释分行，取出最后一行
    buffer = all_comments.back();
    io::Tokenizer::ParseComment(buffer, &buffer);
    size_t pos = buffer.find(' ');
    if (pos == ::std::string::npos)return "";
    result = buffer.substr(0, pos);
    if (result != "ResponseType")return "";
